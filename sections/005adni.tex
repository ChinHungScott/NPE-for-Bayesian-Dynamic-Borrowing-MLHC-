...
As a sensitivity check, we also fit simple Bayesian borrowing comparators on
the cohort-level logit-risk scale. The pooled and fixed power-prior posteriors
were pulled toward the higher-risk ADNI1 cohort, while the commensurate model
and NPE were both centered near the concurrent risk level. NPE had the wider
posterior interval, so we interpret the ADNI result as evidence that the NPE
...
study: direct borrowing from a higher-risk external cohort can shift the target
