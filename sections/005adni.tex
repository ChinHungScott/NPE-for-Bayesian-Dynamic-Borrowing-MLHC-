...
Here $\theta$ is the concurrent risk-level target, $\delta$ represents the
external--concurrent outcome shift, and $\beta$ denotes covariate effects.
The shift parameter allows the external cohort to differ rather than forcing
full exchangeability; it is used during pretraining but is not reported as a
target estimand.
...
This result is consistent with recovering the later-cohort risk level in this
example, but not with claiming greater precision. Appendix~\ref{app:adni-sensitivity}
checks sensitivity to the prior scale for the shift parameter $\delta$.
